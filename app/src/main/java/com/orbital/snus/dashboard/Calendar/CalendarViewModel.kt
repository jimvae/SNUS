...
//    }
    ...
    fun checkIfThisDate(events: List<UserEvent>?, date: Date): ArrayList<UserEvent> {
        ...
        if (events != null) {
            for (event in events) {
                ...
                if ((startDate.compareTo(todayDate) <= 0 && todayDate.compareTo(endDate) <= 0) || fmt.format(
                        todayDate
                    ) == fmt.format(startDate) || fmt.format(todayDate) == fmt.format(endDate)
                )
